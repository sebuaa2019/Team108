...
def str2shell2(str):
    ...
    if str == 'map_start':
        os.system("gnome-terminal -e 'bash -c \"roslaunch wpb_home_bringup minimal.launch; exec bash\"'")
        time.sleep(2)
        p_map = subprocess.Popen('roslaunch wpb_home_tutorials hector_mapping.launch',shell=True)
    ...
    if str == 'map_finish':
        ...
        os.system("gnome-terminal -e 'bash -c \"cp ~/map.pgm ~/catkin_ws/src/team_108/maps/\"'")
        os.system("gnome-terminal -e 'bash -c \"cp ~/map.yaml ~/catkin_ws/src/team_108/maps/\"'")
        os.system("gnome-terminal -e 'bash -c \"cp ~/map.pgm ~/catkin_ws/src/wbp_home/wpb_home_tutorials/maps/\"'")
        os.system("gnome-terminal -e 'bash -c \"cp ~/map.yaml ~/catkin_ws/src/wbp_home/wpb_home_tutorials/maps/\"'")
        os.system("gnome-terminal -e 'bash -c \"rm ~/waypoints.xml\"'")
        time.sleep(3)
        p_map.kill()
    if str == 'move_forward':
        if p_move is None:
    ...
    if 'grab' in str:
        if p_navi is not None:
            if p_navi.poll() is None:
                p_navi.kill()
        if p_grab is not None:
            if p_grab.poll() is None:
                p_grab.kill()
        p_grab = subprocess.Popen('roslaunch team_108 script_add1.launch',shell=True)
    ...
    if 'navigation' in str:
        ...
        if p_grab is not None:
            if p_grab.poll() is None:
                p_grab.kill()
        p_navi = subprocess.Popen('roslaunch team_108 script_add2.launch',shell=True)
...
# counter = 0
# p = subprocess.Popen("ping 192.168.1.1",shell = True)
